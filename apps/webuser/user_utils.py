...
from django_tools.utils import hash_util


def sign_data_for_desktop(data, to_sign):
    """
    scatter desktop桌面版数据hash
    :param to_sign:
    :param data:
    :return:
    """
    a = hash_util.sha256(to_sign)
    b = hash_util.sha256(data)
    return '%s%s' % (a, b)


# def sign_data_for_chrome(data, to_sign):
#     """
#     scatter chrome Extension 数据hash
#     :param data:
#     :param to_sign:
#     :return:
#     """
#     a = hash_util.sha256(to_sign)
#     b = hash_util.sha256(data)
#     return hash_util.sha256('%s%s' % (a, b))
...
# def validate_signature2(msg, sig, pubkey):
#     import ecdsa as ecdsa2
#     key_type, key_string = signature_from_string(sig)
#     a = check_decode(pubkey[3:])
#     vk = ecdsa2.VerifyingKey.from_string(a.hex(), curve=ecdsa2.SECP256k1)
#     res = vk.verify(bytes.fromhex(key_string), msg)
#     return res
